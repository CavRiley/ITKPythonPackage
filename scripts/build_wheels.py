...
def _has_conda_itk() -> bool:
    """Return True if a conda or pixi env has a pre-installed ITK.

    Mirrors :meth:`BuildPythonInstanceBase._detect_conda_itk` but runs
    before any build-instance objects exist so we can skip the ITK
    source clone entirely (the conda package supplies every cmake
    file the build needs).
    """
    import glob as _glob

    for _env_var in ("CONDA_PREFIX", "PIXI_ENVIRONMENT_DIR"):
        _prefix = os.environ.get(_env_var, "")
        if not _prefix:
            continue
        _pattern = os.path.join(_prefix, "lib", "cmake", "ITK-*", "ITKConfig.cmake")
        if _glob.glob(_pattern):
            return True
    return False
...
def build_wheels_main() -> None:
    ...
    # ITK repo handling.  When a conda-installed ITK is on PATH we don't
    # need to clone or fetch anything — the conda package ships all the
    # CMake files the wheel build consumes.
    _conda_itk_available = _has_conda_itk()

    if not Path(args.itk_source_dir).exists():
        if _conda_itk_available:
            print(
                f"Conda ITK detected — skipping ITK source clone "
                f"(ITK_SOURCE_DIR={args.itk_source_dir} does not exist but is not needed). "
                "Stubs will be sourced from the conda prefix."
            )
            # Create an empty placeholder so downstream path checks don't raise.
            args.itk_source_dir.mkdir(parents=True, exist_ok=True)
            # scikit-build-core's pyproject_metadata reader fails with
            # "Readme file not found" if pyproject.toml's readme = ITK/README.md
            # points at a missing file.  Write a minimal placeholder so the
            # wheel build proceeds; the metadata isn't user-facing for the
            # conda-cache path (we're consuming pre-built artifacts).
            readme_placeholder = args.itk_source_dir / "README.md"
            if not readme_placeholder.exists():
                readme_placeholder.write_text(
                    "# ITK (conda-cache placeholder)\n\n"
                    "This file is a placeholder generated by ITKPythonPackage "
                    "when the build consumes a conda-supplied ITK and so does "
                    "not clone the ITK source tree.  The real ITK readme is "
                    "shipped with the ITK GitHub repository.\n"
                )
        else:
    ...
    if not _conda_itk_available or (args.itk_source_dir / ".git").exists():
        run_commandLine_subprocess(
        ...
        try:
            ...
            print(
                f"WARNING: Failed to checkout {args.itk_git_tag}, reverting to 'main':"
            )
            run_commandLine_subprocess(
                ["git", "checkout", "main"],
                cwd=args.itk_source_dir,
